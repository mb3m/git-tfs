...
namespace Sep.Git.Tfs.Core
{
    public class GitTfsRemote : IGitTfsRemote
    {
        ...
        private IGitTfsRemote FindOrInitTfsRemoteOfChangeset(int parentChangesetId, bool mergeChangeset, IRenameResult renameResult, out string omittedParentBranch)
        {
            ...
            {
                // If the changeset has created multiple items, the expected branch item will not always be the first
                // so we scan all the changes to try to detect the first one which is a branch.
                // In most cases it will change nothing: the first change is the good one
